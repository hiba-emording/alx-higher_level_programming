...
| 2           | 2-answer.txt  | In the following code, do `a` and `b` point to the same object? | `>>> a = 89 >>> b = 100` |
| 3           | 3-answer.txt  | In the following code, do `a` and `b` point to the same object? | `>>> a = 89 >>> b = 89` |
| 4           | 4-answer.txt  | In the following code, do `a` and `b` point to the same object? | `>>> a = 89 >>> b = a` |
| 5           | 5-answer.txt  | In the following code, do `a` and `b` point to the same object? | `>>> a = 89 >>> b = a + 1` |
...
| 31          | 103-line1.txt<br>102-line2.txt | How many `int` objects are created by the execution of the first line in this script? <br> How many `int` objects are created by the execution of the second line in this script? | `a = 1 b = 1` |
| 32          | 104-line1.txt<br>104-line2.txt<br>104-line3.txt<br>104-line4.txt<br>104-line5.txt | How many `int` objects are created by the execution of the first line in this script? <br> How many `int` objects are created by the execution of the second line in this script? <br> After the execution of line 3, is the `int` object pointed to by `a` deleted? <br> After the execution of line 4, is the `int` object pointed to by `b` deleted? <br> How many `int` objects are created by the execution of the last line in this script? | `a = 1024 b = 1024 del a del b c = 1024` |
| 33          | 105-line1.txt | Before the execution of line 2 in this script, how many `int` objects have been created and are still in memory? <br> `print("I") print("Love") print("Python")` | `print("I") print("Love") print("Python")` |
